...
public class ExecuteActivity {
	public static LFInputWrapper execByDBId(ExecuteActivityInputWrapper eaiw, LFInputWrapper LFInput) throws IOException { 
		...
		try {
			...
			try {	
				...
				eaiw.currActivityId = eaiw.activityId; //could be sketchy
				
				//attempt to close down current activity's condition env
				ExecuteCurrentActivityWrapper ecawTop = new ExecuteCurrentActivityWrapper();
				ecawTop.eaiw = eaiw;
				ecawTop.currPrevEnv = eapw.prevEnv;
				CheckActivitySolvingWrapper asw = CheckActivitySolving.exec(eaiw, eapw, ecawTop);
				eaiw = asw.eaiw;
				eapw = asw.eapw;
				ecawTop = asw.ecaw;
...
//				}	
				...
				eaiw.fw.append("ERROR HERE FOR EXECUTION OF ACTIVITY " + eaiw.activityId + ": " + f.getMessage());
				eaiw.fw.flush();
				f.printStackTrace();
			}		
		}
		LFInput.sw.env = eaiw.envIn;
		return LFInput;
