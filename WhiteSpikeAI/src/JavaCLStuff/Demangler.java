...
import JavaCLStuff.TimeT;
import JavaCLStuff.ValuedEnum;
import JavaCLStuff.Convention;
import JavaCLStuff.Convention.Style;
import JavaCLStuff.Name;
import JavaCLStuff.Namespace;
import JavaCLStuff.Ptr;
import JavaCLStuff.Template;
import JavaCLStuff.AnnotationUtils;
        ...
        if (type == long.class || type == Long.class) {
            return annotations == null && Pointer.SIZE == 8 ||
                annotations != null && annotations.isAnnotationPresent(Ptr.class) &&
                    !annotations.isAnnotationPresent(JavaCLStuff.CLong.class);
        }
    ...
    static boolean isCLong(Type type, Class typec, Annotations annotations) {
        if (type == long.class || type == Long.class) {
            return annotations == null && CLong.SIZE == 8 ||
                annotations != null && annotations.isAnnotationPresent(JavaCLStuff.CLong.class);
        }
    ...
    static int getIntegralSize(Type type, Class typec, Annotations annotations) {
        ...
        if (type == long.class || type == Long.class) {
            if (annotations != null) {
                ...
                if (annotations.isAnnotationPresent(JavaCLStuff.CLong.class)) {
                    return CLong.SIZE;
                }
            }
            return 8;
        ...
        static int getArgumentsStackSize(Method method) {
            ...
            for (int iArg = 0, nArgs = paramTypes.length; iArg < nArgs; iArg++) {
                ...
                if (paramType == int.class) {
                    ...
                    if (isAnnotationPresent(Ptr.class, as) || isAnnotationPresent(JavaCLStuff.CLong.class, as)) //if (hasInstance(anns[iArg], Ptr.class, CLong.class))
                    {
                        total += Pointer.SIZE;
                    } else {
                        total += 8;
